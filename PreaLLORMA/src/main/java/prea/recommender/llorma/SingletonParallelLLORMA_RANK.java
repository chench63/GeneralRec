 ...
 * @version 1.2
 */
public class SingletonParallelLLORMA_RANK implements Recommender {
    /*
     * ======================================== Common Variables
     * ========================================
     */
     ...
     */
    public SingletonParallelLLORMA_RANK(int uc, int ic, double max, double min, int fc, double lr,
                                        double r, int iter, int mm, int kt, double kw,
                                        RegularizedSVD base, SparseRowMatrix tm, int ml,
                                        boolean verbose) {
        userCount = uc;
        itemCount = ic;
        maxValue = max;
     ...
     */
    @Override
    public void buildModel(SparseRowMatrix ttMatrix) {
            ...
            if (runningThreadCount < multiThreadCount) {
                ...
                for (int indx = 0; indx < pTestItemCount; indx++) {
                    ...
                    double weight = KernelSmoothing.kernelize(getUserSimilarity(anchorUser[mc], u),
                        kernelWidth, kernelType) * KernelSmoothing.kernelize(
                            getItemSimilarity(anchorItem[mc], i), kernelWidth, kernelType);
                    ...
                    pMatrix.setValue(u, i, prediction);
                ...
                if (showProgress) {
                    EvaluationMetrics metrix = new EvaluationMetrics(ttMatrix, pMatrix, maxValue,
                        minValue);
                    System.out
                        .println((modelCount - multiThreadCount + 1) + "\t"
                                 + learners[mp].getAnchorUser() + "\t"
                                 + learners[mp].getAnchorItem() + "\t" + learners[mp].getTrainErr()
