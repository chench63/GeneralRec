...
public class EvaluationMetrics {
     ...
     *
     */
    public EvaluationMetrics(SparseRowMatrix tm, SparseRowMatrix p, double max, double min) {
        testMatrix = tm;
        predicted = p;
        maxValue = max;
        minValue = min;
    ...
    private void build() {
        ...
        for (int u = 1; u <= userCount; u++) {
            ...
            if (realRateList.itemCount() != predictedRateList.itemCount()) {
                System.out
                    .print("Error: The number of test items and predicted items does not match!");
                continue;
            }
            ...
            if (realRateList.itemCount() > 0) {
                ...
                Sort.quickSort(predictedRateValue, predictedRateIndex, 0,
                    predictedRateIndex.length - 1, false);
                Sort.quickSort(realRateValue, realRateIndex, 0, predictedRateIndex.length - 1,
                    false);
                ...
                for (int i : predictedRateIndex) {
                    ...
                    asymmetricLoss += Loss.asymmetricLoss(realRate, predictedRate, minValue,
                        maxValue);
                    testItemCount++;
                ...
                // Rank-based metrics:
                ndcg += Distance.distanceNDCG(realRateList.indexList(), realRateList.valueList(),
                    predictedRateList.indexList(), predictedRateList.valueList());
                kendallsTau += Distance.distanceKendall(realRateList.indexList(),
                    realRateList.valueList(), predictedRateList.indexList(),
                    predictedRateList.valueList(), realRateList.itemCount());
                spearman += Distance.distanceSpearman(realRateList.indexList(),
                    realRateList.valueList(), predictedRateList.indexList(),
                    predictedRateList.valueList(), realRateList.itemCount());
            }
        }
