...
//					);
	...
	public Itemmatrix getItemmatrixByUser(User usr){
		try {
			...
			for(Iterator<Itemmatrix> iter= instance.iterator();iter.hasNext();)
				System.out.println(iter.next().getItemId());
	...
	public boolean checkExist( Matrix instance ){
		try {
			String hql = " from Matrix matrix  " +
					"where matrix.user.usrId = ? and  " +
					"matrix.itemmatrix.itemId=?  ";
			
			Query query = this.getSession().createQuery(hql);
			query.setParameter(0, instance.getUser().getUsrId());
			query.setParameter(1, instance.getItemmatrix().getItemId());
			log.debug("Get successful");
