...
public class SparseVector implements Serializable {
     ...
     * 
     * @return The dense version of same vector.
     */
    public DenseVector toDenseVector() {
        DenseVector v = new DenseVector(this.N);

        int[] indexList = this.indexList();
        for (int i : indexList) {
     ...
     * 
     * @param indexList The list of indices converting to dense vector.
     * @return The dense representation of same vector, with given indices. 
     */
    public DenseVector toDenseSubset(int[] indexList) {
        if (indexList == null || indexList.length == 0)
            return null;

        DenseVector m = new DenseVector(indexList.length);
