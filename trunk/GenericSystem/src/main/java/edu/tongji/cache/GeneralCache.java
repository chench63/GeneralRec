...
public final class GeneralCache {
    ...
    public static synchronized CacheTask task() {
        if (tasks.isEmpty()) {
            ...
            LoggerUtil.info(logger, "Release Task: " + task.get(CacheTask.FILE));
            return task;
        }
    }
    ...
    protected static class FrequencyUtil {
        ...
        public static void releaseMainMemLessThanMin() {
            ...
            for (int pStart = ((Double) (freArrInMain.size() * 0.95)).intValue(), pEnd = freArrInMain
                .size() - 1; pStart <= pEnd; pEnd--) {
            ...
            //更新最小引用次数
            min_freqncy = freArrInMain.get(freArrInMain.size() - 1).getFreqncy();
        }
        ...
        public static void insetIntoMainMem(int id) {
            ...
            String fileName = NetflixEvaPredctFileReader.loadSimilarityOutter(id, content);
            ...
            //3. 输出日志
            LoggerUtil.info(logger, "M.M       File: " + fileName);
        ...
        public static void insertIntoAuxiliaryMem(int id) {
            ...
            if (auxiliaryCache.size() < ConfigurationConstant.AUXILIARY_MEM_SIZE) {
                ...
                //辅助缓存已满，头部插入，尾部剔除
                auxiliaryCache.remove(ConfigurationConstant.AUXILIARY_MEM_SIZE - 1);
         ...
         * @param x
         */
        protected static void loadSingleArr(List<Number> xArr, List<SimilarityVO> content, int x) {
            xArr.clear();
...
class Frequency implements Comparable<Frequency> {
     ...
     * 
     * @param freqncy value to be assigned to property freqncy
     */
...
class NumberSeq {
     ...
     * 
     * @param numArr value to be assigned to property numArr
     */
