...
public class NetflixEvaPredctRecorder extends Thread {
	...
	public void run() {
		...
		while ((task = GeneralCache.task()) != null) {
			...
			for (int i = 1; i < contents.length; i++) {
				...
				prsnHistry
						.add((RatingVO) TemplateType.NETFLIX_RATINGVO_TEMPLATE
								.parser(template));
			}
			...
			for (RatingVO rating : prsnHistry) {
				...
				statGlobal.addValue(Math.abs(rating.getRatingReal()
						- rating.getRatingCmp()));
				statSingle.addValue(Math.abs(rating.getRatingReal()
						- rating.getRatingCmp()));
			}
			LoggerUtil.info(
					logger,
					new StringBuilder("File: ").append(file.getName())
							.append('\t').append(statSingle));
	...
	protected synchronized void completeAndReport(StatisticsInner statGlobal) {
		...
		if (STAT.getN() == ConfigurationConstant.THREAD_SIZE) {
			LoggerUtil.info(
					logger,
					(new StringBuilder(FileUtil.BREAK_LINE))
							.append("Attention!").append(FileUtil.BREAK_LINE)
							.append("S: ").append(STAT.getSum())
							.append(FileUtil.BREAK_LINE).append("N: ")
							.append(STAT.getN()));
		}
	}
	...
	protected class StatisticsInner {
		...
		public String toString() {
			return (new StringBuilder("Sum: "))
					.append(StringUtil.alignLeft(String.format("%.3f", sum), 12))
					.append(" N: ")
					.append(StringUtil.alignLeft(String.format("%d", N), 12))
					.append(" Avg: ").append(this.getAverage()).toString();
		}
	}
