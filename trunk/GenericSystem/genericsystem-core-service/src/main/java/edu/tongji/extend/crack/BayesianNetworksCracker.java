...
public class BayesianNetworksCracker extends ExpectationCracker {
    ...
    /** 
     * @see edu.tongji.crack.ExpectationSeqDayCracker#crackInnerNoise(edu.tongji.extend.crack.CrackObject, edu.tongji.noise.Noise)
     */
    @SuppressWarnings("unchecked")
    @Override
    public void crackInnerNoise(CrackObject object, Noise noise, HashKeyCallBack hashKyGen) {
        ...
        for (int i = 0, j = baseElems.size(); i < j; i++) {
            ...
            //2. 计算条件概率
            String key = (hashKyGen == null) ? DateUtil.format(new Date(baseElems.get(i)
                .getTimeVal()), DateUtil.SHORT_FORMAT) : hashKyGen.key(baseElems.get(i)
                .getTimeVal());
            double cpVal = BayesianNetworkPRUtil.cp(1, WeatherCache.get(key).getHighTemper(),
                meanEstmt);
            CP_RESULT.add(cpVal);
