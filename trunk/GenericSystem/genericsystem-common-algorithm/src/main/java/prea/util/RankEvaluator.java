...
public class RankEvaluator {
        ...
        for (int u = 1; u <= userCount; u++) {
            ...
            if (testItems != null) {
                for (int i : testItems) {
                    ...
                    double predictedRate_i = usePredicted ? predicted.getValue(u, i) : U.getRowRef(
                        u).innerProduct(V.getColRef(i));

                    predictedItems.setValue(i, predictedRate_i);

                    for (int j : testItems) {
                        double realRate_j = testMatrix.getValue(u, j);
                        double predictedRate_j = usePredicted ? predicted.getValue(u, j) : U
                            .getRowRef(u).innerProduct(V.getColRef(j));

                        if (realRate_i > realRate_j) {
                            for (int l = 0; l < LOSS_COUNT; l++) {
                                userLoss[l] += loss(realRate_i, realRate_j, predictedRate_i,
                                    predictedRate_j, l);
                            }
                            pairCount++;
                        }
            ...
            if (observedItems.itemCount() > 0) {
                ...
                for (int i = 0; i < listLength; i++) {
                    double observed = rateMatrix.getValue(u, observedIndices[i])
                                      + testMatrix.getValue(u, observedIndices[i]);
    ...
    public static double loss(double Mui, double Muj, double Fui, double Fuj, int lossCode) {
        switch (lossCode) {
            ...
            case SMOOTH_L1_REGRESSION:
                return Math.log(1 + Math.exp(Mui - Muj - Fui + Fuj))
                       + Math.log(1 + Math.exp(Fui - Fuj - Mui + Muj));
            default:
                return 0.0;
        }
    }
    ...
    public static double lossDiff(double Mui, double Muj, double Fui, double Fuj, int lossCode) {
        switch (lossCode) {
            case LOGISTIC_LOSS:
                return (Muj - Mui) * Math.exp((Mui - Muj) * (Fui - Fuj))
                       / Math.pow(1 + Math.exp((Mui - Muj) * (Fui - Fuj)), 2);
            case DISCRETE_LOSS:
                return 0.0;
            case LOG_LOSS_MULT:
            ...
            case SMOOTH_L1_REGRESSION:
                return -1.0 / (1 + Math.exp(Fui - Fuj - Mui + Muj)) + 1.0
                       / (1 + Math.exp(Mui - Muj - Fui + Fuj));
            default:
                return 0.0;
        }
    }
    ...
    public String printOneLine() {
        return String.format(
            ...
            this.getLoss(LOGISTIC_LOSS), this.getLoss(LOG_LOSS_MULT), this.getLoss(LOG_LOSS_ADD),
            this.getLoss(EXP_LOSS_MULT), this.getLoss(EXP_LOSS_ADD), this.getLoss(HINGE_LOSS_MULT),
            this.getLoss(HINGE_LOSS_ADD), this.getLoss(ABSOLUTE_LOSS), this.getLoss(SQUARED_LOSS),
            this.getLoss(EXP_REGRESSION), this.getLoss(SMOOTH_L1_REGRESSION), this.getNDCG(),
    ...
    public static String printTitle() {
        return "=====================================================================================================================================================================\r\nName\tOptLoss\tRank\tLogs\tLog/M\tLog/A\tExp/M\tExp/A\tHinge/M\tHinge/A\tAbs\tSqr\tExpReg\tSmL1\tNDCG@"
               + NDCG_THRESHOLD + "\t0/1";
    }
}
